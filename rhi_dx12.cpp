...
namespace rhi {
	...
	namespace {
		...
		static void Dx12AppendUniqueExistingDirectory(std::vector<std::string>& directories, const std::filesystem::path& candidate) {
			std::error_code error;
			if (candidate.empty() || !std::filesystem::exists(candidate, error) || !std::filesystem::is_directory(candidate, error)) {
				return;
			}
			...
			auto equalsIgnoreCase = [&](const std::string& existing) {
				return existing.size() == normalized.size()
					&& std::equal(existing.begin(), existing.end(), normalized.begin(), [](char left, char right) {
						return std::tolower(static_cast<unsigned char>(left)) == std::tolower(static_cast<unsigned char>(right));
					});
			};

			if (std::find_if(directories.begin(), directories.end(), equalsIgnoreCase) == directories.end()) {
	...
	namespace {
		...
		static void Dx12EnsureReShapePipelineInventory(Dx12Device* impl) noexcept {
...

			{
				...
				impl->debugInstrumentation.pipelineInventoryRefreshRequested = true;
			}
		}
		#else
		static void Dx12EnsureReShapePipelineInventory(Dx12Device* impl) noexcept {
	...
	namespace {
		...
		static std::string Dx12ExtractSourceLineText(std::string_view source, uint32_t zeroBasedLine) {
			if (source.empty()) {
				return {};
			}

			size_t lineStart = 0;
			for (uint32_t lineIndex = 0; lineIndex < zeroBasedLine; ++lineIndex) {
		...
		static std::string Dx12NormalizeSourceLineForMatching(std::string_view sourceLine) {
			const size_t first = sourceLine.find_first_not_of(" \t\r\n");
			if (first == std::string_view::npos) {
				return {};
			}

			const size_t last = sourceLine.find_last_not_of(" \t\r\n");
			return std::string(sourceLine.substr(first, last - first + 1u));
		}

		static bool Dx12DoesMappedSourceLineMatch(
			std::string_view sourceContents,
			uint32_t zeroBasedLine,
			std::string_view expectedSourceLine) {
			if (sourceContents.empty() || expectedSourceLine.empty()) {
				return false;
			}

			return Dx12NormalizeSourceLineForMatching(Dx12ExtractSourceLineText(sourceContents, zeroBasedLine))
				== Dx12NormalizeSourceLineForMatching(expectedSourceLine);
		}
		...
		struct Dx12MappedShaderFileResolution {
			bool resolved = false;
			uint32_t resolvedFileUid = kDx12InvalidShaderSourceFileUid;
			bool resolvedLineMatched = false;
			bool requestedFileUidHadCandidate = false;
			bool requestedFileUidLineMatched = false;
			std::string requestedFilePath;
			std::string resolvedPath;
			std::string resolvedContents;
		};
		...
		static void Dx12AppendShaderResolutionInfo(
			...
			std::snprintf(
				message,
				sizeof(message),
				"Shader resolution: SGUID %llu -> shader %llu, fileUID %u, line %u, attempt %u (%s)",
				static_cast<unsigned long long>(sguid),
				static_cast<unsigned long long>(shaderGuid),
				fileUid,
				line,
				requestAttempts,
				reason ? reason : "") ;
		...
		static void Dx12AppendShaderCodeInfo(
			Dx12Device* impl,
			uint64_t shaderUid,
			bool found,
			bool native,
			...
			std::snprintf(
				message,
				sizeof(message),
				"Shader code response: shader %llu, found=%s, native=%s, fileCount=%u",
				static_cast<unsigned long long>(shaderUid),
				found ? "true" : "false",
				native ? "true" : "false",
				fileCount);
		...
		static void Dx12AppendShaderCodeFileInfo(
			Dx12Device* impl,
			uint64_t shaderUid,
			uint32_t fileUid,
			...
			std::snprintf(
				message,
				sizeof(message),
				"Shader file response: shader %llu, fileUID %u, filename=%.*s",
				static_cast<unsigned long long>(shaderUid),
				fileUid,
		...
		static void Dx12PopulateShaderFileCandidate(
			...
			const auto contentsIt = metadata.fileContentsByUid.find(fileUid);
			if (contentsIt != metadata.fileContentsByUid.end() && !contentsIt->second.empty()) {
				sourceContents = contentsIt->second;
				if (sourcePath.empty()) {
					sourcePath = Dx12BuildSyntheticShaderSourcePath(shaderUid, fileUid);
				}
			}
		}

		static Dx12MappedShaderFileResolution Dx12AnalyzeMappedShaderFileResolution(
			const Dx12ShaderIssueMetadata& metadata,
			uint64_t shaderUid,
			const Dx12ShaderSourceMappingMetadata& mapping) {
			Dx12MappedShaderFileResolution result{};

			std::unordered_set<uint32_t> candidateFileUids;
			if (mapping.fileUid != kDx12InvalidShaderSourceFileUid) {
				candidateFileUids.insert(mapping.fileUid);
			}
			for (uint32_t fileUid : metadata.fileUids) {
				candidateFileUids.insert(fileUid);
			}
			for (const auto& [fileUid, path] : metadata.filePathsByUid) {
				(void)path;
				candidateFileUids.insert(fileUid);
			}
			...
			for (uint32_t candidateFileUid : candidateFileUids) {
				std::string candidatePath;
				std::string candidateContents;
				Dx12PopulateShaderFileCandidate(metadata, shaderUid, candidateFileUid, candidatePath, candidateContents);
				if (candidatePath.empty() && candidateContents.empty()) {
					continue;
				}

				const bool lineMatched = mapping.sourceLine.empty()
					? (candidateFileUid == mapping.fileUid)
					: Dx12DoesMappedSourceLineMatch(candidateContents, mapping.line, mapping.sourceLine);

				if (candidateFileUid == mapping.fileUid) {
					result.requestedFileUidHadCandidate = true;
					result.requestedFileUidLineMatched = lineMatched;
					result.requestedFilePath = candidatePath;
				}
				...
				if (lineMatched) {
					if (candidateFileUid == mapping.fileUid) {
						rank = 0;
					} else if (!candidatePath.empty() && Dx12IsPrimaryShaderSourcePath(candidatePath)) {
						rank = 1;
					} else {
						rank = 2;
					}
				} else if (candidateFileUid == mapping.fileUid) {
					rank = 3;
				}
				...
				if (rank < bestRank) {
					...
					result.resolvedFileUid = candidateFileUid;
					result.resolvedLineMatched = lineMatched;
					result.resolvedPath = std::move(candidatePath);
					result.resolvedContents = std::move(candidateContents);
				}
			}
		...
		static bool Dx12TryResolveMappedShaderFile(
			const Dx12ShaderIssueMetadata& metadata,
			uint64_t shaderUid,
			const Dx12ShaderSourceMappingMetadata& mapping,
			std::string& sourcePath,
			std::string& sourceContents) {
			Dx12MappedShaderFileResolution resolution = Dx12AnalyzeMappedShaderFileResolution(metadata, shaderUid, mapping);
			sourcePath = std::move(resolution.resolvedPath);
			sourceContents = std::move(resolution.resolvedContents);
			return resolution.resolved;
		}

		static void Dx12AppendShaderFileResolutionTelemetry(
			Dx12Device* impl,
			Dx12DebugInstrumentationSession& session,
			uint64_t sguid,
			uint64_t shaderUid,
			const Dx12ShaderSourceMappingMetadata& mapping,
			const Dx12MappedShaderFileResolution& resolution) {
			if (!impl || sguid == 0 || !resolution.resolved) {
				return;
			}

			const bool suspicious = (mapping.fileUid != kDx12InvalidShaderSourceFileUid)
				&& (!resolution.requestedFileUidHadCandidate
					|| !resolution.requestedFileUidLineMatched
					|| resolution.resolvedFileUid != mapping.fileUid);
			if (!suspicious) {
				return;
			}

			if (!session.suspiciousShaderFileResolutionTelemetrySguids.insert(sguid).second) {
				return;
			}

			const std::string normalizedSourceLine = Dx12NormalizeSourceLineForMatching(mapping.sourceLine);
			char message[768] = {};
			std::snprintf(
				message,
				sizeof(message),
				"Shader file resolution telemetry: SGUID %llu shader %llu mapped fileUID %u line %u resolved to fileUID %u; requestedCandidate=%s requestedLineMatch=%s resolvedLineMatch=%s mappedPath=%.*s resolvedPath=%.*s sourceLine=%.*s",
				static_cast<unsigned long long>(sguid),
				static_cast<unsigned long long>(shaderUid),
				mapping.fileUid,
				mapping.line,
				resolution.resolvedFileUid,
				resolution.requestedFileUidHadCandidate ? "true" : "false",
				resolution.requestedFileUidLineMatched ? "true" : "false",
				resolution.resolvedLineMatched ? "true" : "false",
				static_cast<int>((std::min)(resolution.requestedFilePath.size(), static_cast<size_t>(120))),
				resolution.requestedFilePath.data(),
				static_cast<int>((std::min)(resolution.resolvedPath.size(), static_cast<size_t>(120))),
				resolution.resolvedPath.data(),
				static_cast<int>((std::min)(normalizedSourceLine.size(), static_cast<size_t>(120))),
				normalizedSourceLine.data());
			Dx12AppendInstrumentationDiagnostic(impl, DebugInstrumentationDiagnosticSeverity::Warning, message, false);
		}
		...
		static bool Dx12ExecutionDetailHasPathUnlocked(
			...
			if (detail.sguid != 0) {
				...
				if (mappingIt != session.shaderSourceMappings.end() && mappingIt->second.resolved && metadataIt != session.shaderMetadata.end()) {
					std::string resolvedPath;
					std::string resolvedContents;
					if (Dx12TryResolveMappedShaderFile(metadataIt->second, resolvedShaderUid, mappingIt->second, resolvedPath, resolvedContents)
			...
			if (detail.sguid != 0) {
				...
				if (mappingIt != session.shaderSourceMappings.end() && mappingIt->second.resolved) {
					...
					if (metadataIt != session.shaderMetadata.end()) {
						...
						Dx12TryResolveMappedShaderFile(
							metadataIt->second,
							shaderUid,
							mappingIt->second,
							snapshot.sourcePath,
		...
		static void Dx12RebuildInstrumentationIssuesUnlocked(Dx12Device* impl, Dx12DebugInstrumentationSession& session) {
			...
			for (const Dx12PendingInstrumentationShaderIssue& pendingIssue : session.pendingShaderIssues) {
				...
				if (metadataIt != session.shaderMetadata.end()) {
					...
					if (resolvedMapping) {
						Dx12MappedShaderFileResolution resolution = Dx12AnalyzeMappedShaderFileResolution(
							metadataIt->second,
							resolvedShaderUid,
							*resolvedMapping);
						resolvedFilePath = resolution.resolvedPath;
						Dx12AppendShaderFileResolutionTelemetry(
							impl,
							session,
							pendingIssue.sguid,
							resolvedShaderUid,
				...
				if (resolvedFilePath.empty()) {
					...
					if (Dx12HasShaderEntryPointLabel(shaderEntryPoint)) {
						Dx12CopyDebugString(issue.label, sizeof(issue.label), shaderEntryPoint.c_str());
					} else {
						char label[64] = {};
				...
				Dx12CopyDebugString(issue.path, sizeof(issue.path), resolvedFilePath.c_str());
				Dx12CopyDebugString(issue.message, sizeof(issue.message), pendingIssue.message.c_str());
				tryAppendIssue(issue);
			}
		...
		static void Dx12QueueShaderSourceMappingRequestUnlocked(Dx12DebugInstrumentationSession& session, uint64_t sguid) noexcept {
			if (!sguid || sguid == kDx12InvalidShaderSguid) {
				return;
			}

			auto& mapping = session.shaderSourceMappings[sguid];
			if (mapping.requestAttempts >= kDx12MaxShaderSourceMappingRequestAttempts
				|| mapping.requested
				|| session.requestedShaderSourceMappings.count(sguid)
				|| session.pendingShaderSourceMappingRequests.count(sguid)) {
				return;
			}
		...
		static void Dx12ProcessReShapeMessageStream(Dx12Device* impl, MessageStream& stream, MessageStream& requestStream) noexcept {
			...
			if (schema == OrderedMessageSchema::GetSchema()) {
				...
				for (auto it = view.GetIterator(); it; ++it) {
					...
					case ObjectStatesMessage::kID: {
						auto* message = it.Get<ObjectStatesMessage>();
						if (!message) {
							break;
						}
					...
					case PipelineUIDRangeMessage::kID: {
						auto* message = it.Get<PipelineUIDRangeMessage>();
						if (!message) {
							break;
						}
					...
					case PipelineStatusCollectionMessage::kID: {
						auto* message = it.Get<PipelineStatusCollectionMessage>();
						if (!message) {
							break;
						}
					...
					case ShaderCodeFileMessage::kID: {
...

						{
							...
							if (!sourceContents.empty()) {
								auto contentsIt = metadata.fileContentsByUid.find(message->fileUID);
								if (contentsIt == metadata.fileContentsByUid.end() || contentsIt->second != sourceContents) {
									metadata.fileContentsByUid[message->fileUID] = sourceContents;
									metadataChanged = true;
								}
							}
			...
			if (stream.Is<ShaderSourceMappingMessage>()) {
				...
				for (auto it = view.GetIterator(); it; ++it) {
					const bool hasUsableSourceMapping = it->shaderGUID != 0
						|| it->fileUID != kDx12InvalidShaderSourceFileUid
						|| !it->contents.Empty();
					uint32_t requestAttempts = 0;
					bool queuedRetry = false;
					uint64_t diagnosticShaderGuid = it->shaderGUID;
					uint32_t diagnosticFileUid = it->fileUID;
					uint32_t diagnosticLine = it->line;
					const char* diagnosticReason = nullptr;
					{
						std::lock_guard guard(impl->debugInstrumentation.mutex);
						Dx12ShaderSourceMappingMetadata& mapping = impl->debugInstrumentation.shaderSourceMappings[it->sguid];
						const bool hadUsableMapping = mapping.resolved
							&& (mapping.shaderGuid != 0
								|| mapping.fileUid != kDx12InvalidShaderSourceFileUid
								|| !mapping.sourceLine.empty());
						mapping.requested = true;
						if (hasUsableSourceMapping || !hadUsableMapping) {
							mapping.resolved = hasUsableSourceMapping;
							mapping.shaderGuid = it->shaderGUID;
							mapping.fileUid = it->fileUID;
							mapping.line = it->line;
							mapping.column = it->column;
							mapping.basicBlockId = it->basicBlockId;
							mapping.instructionIndex = it->instructionIndex;
							mapping.sourceLine.assign(it->contents.View());
							diagnosticReason = hasUsableSourceMapping
								? (it->fileUID == kDx12InvalidShaderSourceFileUid ? "source mapping resolved without file uid" : "source mapping resolved")
								: nullptr;
						} else {
							diagnosticReason = "ignored unresolved response; preserved existing mapping";
						}
						requestAttempts = mapping.requestAttempts;
						diagnosticShaderGuid = mapping.shaderGuid;
						diagnosticFileUid = mapping.fileUid;
						diagnosticLine = mapping.line;
						if (mapping.shaderGuid != 0) {
							Dx12ShaderIssueMetadata& metadata = impl->debugInstrumentation.shaderMetadata[mapping.shaderGuid];
							if (!metadata.requested) {
								impl->debugInstrumentation.pendingShaderCodeRequests.insert(mapping.shaderGuid);
							}
						}
						if (!hasUsableSourceMapping && !hadUsableMapping) {
							impl->debugInstrumentation.requestedShaderSourceMappings.erase(it->sguid);
							queuedRetry = mapping.requestAttempts < kDx12MaxShaderSourceMappingRequestAttempts;
							mapping.requested = !queuedRetry;
							if (queuedRetry) {
								impl->debugInstrumentation.pendingShaderSourceMappingRequests.insert(it->sguid);
							}
							diagnosticReason = queuedRetry ? "source mapping unresolved, retry queued" : "source mapping unresolved";
						}
						impl->debugInstrumentation.issuesDirty = true;
					}
					Dx12AppendShaderResolutionInfo(
						impl,
						it->sguid,
						diagnosticShaderGuid,
						diagnosticFileUid,
						diagnosticLine,
						requestAttempts,
						diagnosticReason ? diagnosticReason : "source mapping updated");
				}
				return;
			}
		...
		static void Dx12PollReShapeMessages(Dx12Device* impl) noexcept {
			...
			{
				std::lock_guard guard(impl->debugInstrumentation.mutex);
				if (impl->debugInstrumentation.pollInProgress) {
					return;
				}
				impl->debugInstrumentation.pollInProgress = true;
			}

			struct PollScopeReset {
				Dx12DebugInstrumentationSession& session;
				~PollScopeReset() {
					std::lock_guard guard(session.mutex);
					session.pollInProgress = false;
				}
			} pollScopeReset{ impl->debugInstrumentation };

			bool shouldContinuePolling = true;
			uint32_t pollRound = 0;
			while (shouldContinuePolling && pollRound < kMaxPollRoundsPerCall) {
				++pollRound;
				...
				{
					std::lock_guard guard(impl->debugInstrumentation.mutex);
					Dx12RebuildInstrumentationIssuesUnlocked(impl, impl->debugInstrumentation);
				}

				{
					...
					for (uint64_t sguid : impl->debugInstrumentation.pendingShaderSourceMappingRequests) {
						...
						auto& mapping = impl->debugInstrumentation.shaderSourceMappings[sguid];
						mapping.requested = true;
						++mapping.requestAttempts;
					}
					impl->debugInstrumentation.pendingShaderSourceMappingRequests.clear();
				}

				issuedRequests = requestStream.GetByteSize() != 0;
				if (issuedRequests) {
					const Result result = Dx12CommitReShapeMessages(impl, requestStream);
					shouldContinuePolling = IsOk(result);
				}
			}
		}
		...
		static bool Dx12InitializeReShapeRuntime(Dx12Device* impl, const DeviceCreateInfo& ci) noexcept {
			...
			if (auto startupContainer = runtime->environment.GetRegistry()->Get<::Backend::StartupContainer>()) {
				auto startupView = MessageStreamView<>(startupContainer->stream);
				auto* texelAddressingMessage = startupView.Add<SetTexelAddressingMessage>();
				texelAddressingMessage->enabled = ci.instrumentation.enableTexelAddressing ? 1u : 0u;
				Dx12AppendInstrumentationDiagnostic(
					impl,
					DebugInstrumentationDiagnosticSeverity::Info,
					ci.instrumentation.enableTexelAddressing
						? "GPU-Reshape startup config enabled texel addressing for the in-process BasicRHI runtime."
						: "GPU-Reshape startup config disabled texel addressing for the in-process BasicRHI runtime.");

				auto* externalReferenceMessage = startupView.Add<SetUseShaderExternalReferenceMessage>();
				externalReferenceMessage->enabled = 1;
				Dx12AppendInstrumentationDiagnostic(
					impl,
					DebugInstrumentationDiagnosticSeverity::Info,
					"GPU-Reshape startup config enabled external shader references for metadata queries.");

				std::vector<std::string> pdbSearchRoots;
				const std::filesystem::path executableDirectory = std::filesystem::path(Dx12GetExecutableDirectory());
				Dx12AppendUniqueExistingDirectory(pdbSearchRoots, executableDirectory / "CompiledShaders");
				{
					std::error_code error;
					const std::filesystem::path currentDirectory = std::filesystem::current_path(error);
					if (!error) {
						Dx12AppendUniqueExistingDirectory(pdbSearchRoots, currentDirectory / "CompiledShaders");
					}
				}

				if (!pdbSearchRoots.empty()) {
					auto* pdbConfigMessage = startupView.Add<SetPDBConfigMessage>();
					pdbConfigMessage->pathCount = static_cast<uint32_t>(pdbSearchRoots.size());
					pdbConfigMessage->recursive = 0;

					for (uint32_t index = 0; index < pdbSearchRoots.size(); ++index) {
						auto* pdbPathMessage = startupView.Add<SetPDBPathMessage>(SetPDBPathMessage::AllocationInfo{
							.pathLength = pdbSearchRoots[index].size()
						});
						pdbPathMessage->index = index;
						pdbPathMessage->path.Set(pdbSearchRoots[index]);
					}

					auto* indexPdbPathsMessage = startupView.Add<IndexPDPathsMessage>();
					indexPdbPathsMessage->ignore = 0;

					for (const std::string& path : pdbSearchRoots) {
						std::string diagnostic = "GPU-Reshape startup config added PDB search root: ";
						diagnostic += path;
						Dx12AppendInstrumentationDiagnostic(impl, DebugInstrumentationDiagnosticSeverity::Info, diagnostic.c_str());
					}
				} else {
					Dx12AppendInstrumentationDiagnostic(
						impl,
						DebugInstrumentationDiagnosticSeverity::Warning,
						"GPU-Reshape startup config did not find a CompiledShaders directory to index for external PDB lookup.");
				}
			}
			...
			MessageStream zeroMaskSpecializationStream;
			Dx12BuildDefaultInstrumentationSpecialization(zeroMaskSpecializationStream);
			auto* zeroMaskMessage = MessageStreamView<>(configStream).Add<SetGlobalInstrumentationMessage>(SetGlobalInstrumentationMessage::AllocationInfo{
				.specializationByteSize = zeroMaskSpecializationStream.GetByteSize()
			});
			zeroMaskMessage->featureBitSet = 0;
			zeroMaskMessage->specialization.Set(zeroMaskSpecializationStream);
			if (Dx12CommitReShapeMessages(impl, configStream) != Result::Ok) {
				Dx12AppendInstrumentationDiagnostic(impl, DebugInstrumentationDiagnosticSeverity::Warning, "GPU-Reshape startup instrumentation configuration did not complete cleanly; the backend may not have honored the default zero feature mask.");
			}
		...
		static void Dx12InitializeDebugInstrumentation(Dx12Device* impl, const DeviceCreateInfo& ci) noexcept {
			...
			instrumentation.state.texelAddressingEnabled = ci.instrumentation.enableTexelAddressing;
		...
		static uint32_t d_getDebugInstrumentationPipelineCount(const Device* d) noexcept {
			if (!d) {
				return 0;
			}
			...
			Dx12EnsureReShapePipelineInventory(impl);
			std::lock_guard guard(impl->debugInstrumentation.mutex);
			return static_cast<uint32_t>(impl->debugInstrumentation.pipelineOrder.size());
		}

		static Result d_copyDebugInstrumentationPipelines(const Device* d,
			uint32_t first,
			...
			Dx12EnsureReShapePipelineInventory(impl);
			std::lock_guard guard(impl->debugInstrumentation.mutex);
			const auto& pipelineOrder = impl->debugInstrumentation.pipelineOrder;
			if (first >= pipelineOrder.size() || capacity == 0 || !outPipelines) {
				return Result::Ok;
			}

			const uint32_t available = static_cast<uint32_t>(pipelineOrder.size() - first);
			const uint32_t toCopy = (std::min)(capacity, available);
			for (uint32_t index = 0; index < toCopy; ++index) {
				const uint64_t pipelineUid = pipelineOrder[first + index];
				DebugInstrumentationPipeline pipeline{};
				pipeline.pipelineUid = pipelineUid;
				auto metadataIt = impl->debugInstrumentation.pipelines.find(pipelineUid);
				if (metadataIt != impl->debugInstrumentation.pipelines.end()) {
					const auto& metadata = metadataIt->second;
					pipeline.kind = metadata.kind;
					pipeline.active = metadata.active;
					pipeline.instrumented = metadata.instrumented;
					pipeline.explicitlyInstrumented = metadata.explicitlyInstrumented;
					pipeline.explicitFeatureMask = metadata.explicitFeatureMask;
					Dx12CopyDebugString(pipeline.label, sizeof(pipeline.label), metadata.label.c_str());
					Dx12CopyDebugString(pipeline.lastPassName, sizeof(pipeline.lastPassName), metadata.lastPassName.c_str());
				}
				outPipelines[index] = pipeline;
			}
		...
		static uint32_t d_getDebugInstrumentationPipelineUsageCount(const Device* d) noexcept {
			if (!d) {
				return 0;
			}
			...
			Dx12EnsureReShapePipelineInventory(impl);
			std::lock_guard guard(impl->debugInstrumentation.mutex);
			return static_cast<uint32_t>(impl->debugInstrumentation.pipelineUsages.size());
		}

		static Result d_copyDebugInstrumentationPipelineUsages(const Device* d,
			uint32_t first,
			...
			Dx12EnsureReShapePipelineInventory(impl);
			std::lock_guard guard(impl->debugInstrumentation.mutex);
			const auto& usages = impl->debugInstrumentation.pipelineUsages;
			if (first >= usages.size() || capacity == 0 || !outUsages) {
				return Result::Ok;
			}

			const uint32_t available = static_cast<uint32_t>(usages.size() - first);
			const uint32_t toCopy = (std::min)(capacity, available);
			for (uint32_t index = 0; index < toCopy; ++index) {
				DebugInstrumentationPipelineUsage usage{};
				usage.pipelineUid = usages[first + index].pipelineUid;
				Dx12CopyDebugString(usage.techniquePath, sizeof(usage.techniquePath), usages[first + index].techniquePath.c_str());
				Dx12CopyDebugString(usage.passName, sizeof(usage.passName), usages[first + index].passName.c_str());
				outUsages[index] = usage;
			}
		...
		static Result d_setDebugPipelineInstrumentationMask(Device* d, uint64_t pipelineUid, uint64_t featureMask) noexcept {
			if (!d || pipelineUid == 0) {
				RHI_FAIL(Result::InvalidArgument);
			}
			...
			MessageStream specializationStream;
			Dx12BuildDefaultInstrumentationSpecialization(specializationStream);
			auto* message = MessageStreamView<>(stream).Add<SetPipelineInstrumentationMessage>(SetPipelineInstrumentationMessage::AllocationInfo{
				.specializationByteSize = specializationStream.GetByteSize()
			});
			message->pipelineUID = pipelineUid;
			message->featureBitSet = featureMask;
		...
		static Result d_setDebugTexelAddressing(Device* d, bool enabled) noexcept {
			if (!d) {
				RHI_FAIL(Result::InvalidArgument);
			}
			...
			Dx12AppendInstrumentationDiagnostic(
				impl,
				DebugInstrumentationDiagnosticSeverity::Warning,
				"Texel addressing selects the GPU-Reshape initialization/concurrency feature set at device creation. Change the startup setting and recreate the device to apply a different mode.",
				false);
			RHI_FAIL(Result::Unsupported);
		}
		...
		static void cl_bindPipeline(CommandList* cl, PipelineHandle psoH) noexcept {
			...
			if (auto* P = dev->pipelines.get(psoH)) {
				...
				if (dev->debugInstrumentation.state.active && P->pso) {
					...
					if (SUCCEEDED(P->pso->QueryInterface(__uuidof(::PipelineState), reinterpret_cast<void**>(&pipelineState)))
						&& pipelineState
						...
						Dx12ObservePipelineUsageUnlocked(
							...
							pipelineState->uid,
							Dx12MapReShapePipelineKind(static_cast<uint32_t>(pipelineState->type)),
							pipelineState->debugName,
							l->instrumentationPassName.c_str(),
							l->instrumentationTechniquePath.c_str());
						dev->debugInstrumentation.pendingPipelineStatusRequests.insert(pipelineState->uid);
						if (pipelineState->debugName && *pipelineState->debugName) {
							dev->debugInstrumentation.pipelineNames[pipelineState->uid] = pipelineState->debugName;
						} else if (dev->debugInstrumentation.pipelineNames.find(pipelineState->uid) == dev->debugInstrumentation.pipelineNames.end()) {
							dev->debugInstrumentation.pendingPipelineNameRequests.insert(pipelineState->uid);
						}
					}
				}
				#endif
		...
		static void cl_setDebugInstrumentationContext(CommandList* cl, const char* passName, const char* techniquePath) noexcept {
			auto* impl = dx12_detail::CL(cl);
			if (!impl) {
				return;
			}

			impl->instrumentationPassName = passName ? passName : "";
			impl->instrumentationTechniquePath = techniquePath ? techniquePath : "";
		}
